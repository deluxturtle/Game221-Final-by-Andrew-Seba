...
public class EngineWindowEditor : EditorWindow {
    ...
    void MovementGUI()
    {
        ...
        switch (movements[movementFocus].moveType)
        {
            ...
            case MovementTypes.LOOKAT:
                #region move lookat inspector
                //target
                if (movements[movementFocus].targets == null || movements[movementFocus].targets.Length < 1)
                {
                    movements[movementFocus].targets = new GameObject[1];
                }
                if (movements[movementFocus].lockTimes == null || movements[movementFocus].lockTimes.Length < 1)
                {
                    movements[movementFocus].lockTimes = new float[1];
                }
                if (movements[movementFocus].rotationSpeed == null || movements[movementFocus].rotationSpeed.Length < 2)
                {
                    movements[movementFocus].rotationSpeed = new float[2];
                }
                windowDisplay = new Rect(offsetX, offsetY, 120f, DISPLAY_HEIGHT);
                offsetX += 120;
                ...
                offsetX = 15;
                offsetY += DISPLAY_DIF;
                movements[movementFocus].targets[0] = (GameObject)EditorGUI.ObjectField(windowDisplay, GUIContent.none, movements[movementFocus].targets[0], typeof(GameObject), true);
                //facing time
                ...
                offsetX += 30f;
                movements[movementFocus].rotationSpeed[0] = EditorGUI.FloatField(windowDisplay, movements[movementFocus].rotationSpeed[0]);
                windowDisplay = new Rect(offsetX, offsetY, 30f, DISPLAY_HEIGHT);
                offsetX = 15;
                offsetY += DISPLAY_DIF;
                EditorGUI.LabelField(windowDisplay, "secs");
                ...
                windowDisplay = new Rect(offsetX, offsetY, 30f, DISPLAY_HEIGHT);
                offsetX += 30f;
                movements[movementFocus].lockTimes[0] = EditorGUI.FloatField(windowDisplay, movements[movementFocus].lockTimes[0]);
                windowDisplay = new Rect(offsetX, offsetY, 30f, DISPLAY_HEIGHT);
                offsetX = 15;
                offsetY += DISPLAY_DIF;
                EditorGUI.LabelField(windowDisplay, "secs");
                ...
                windowDisplay = new Rect(offsetX, offsetY, 30f, DISPLAY_HEIGHT);
                offsetX += 30f;
                movements[movementFocus].rotationSpeed[1] = EditorGUI.FloatField(windowDisplay, movements[movementFocus].rotationSpeed[1]);
                windowDisplay = new Rect(offsetX, offsetY, 30f, DISPLAY_HEIGHT);
                offsetX = 15;
                offsetY += DISPLAY_DIF;
                EditorGUI.LabelField(windowDisplay, "secs");
                ...
                //Seba
                movements[movementFocus].movementTime = movements[movementFocus].lockTimes[0] + movements[movementFocus].rotationSpeed[1] + movements[movementFocus].rotationSpeed[0];
                #endregion
                break;
            case MovementTypes.LOOKCHAIN:
                ...
                offsetX = 15f;
                offsetY += DISPLAY_DIF;
                movements[movementFocus].targetSize = EditorGUI.IntField(windowDisplay, movements[movementFocus].targetSize);
                if (movements[movementFocus].targets != null && movements[movementFocus].targets.Length != movements[movementFocus].targetSize)
                {
                    movements[movementFocus].targets = new GameObject[movements[movementFocus].targetSize];
                }
                if (movements[movementFocus].lockTimes != null && movements[movementFocus].lockTimes.Length != movements[movementFocus].targetSize)
                {
                    movements[movementFocus].lockTimes = new float[movements[movementFocus].targetSize];
                }
                Debug.Log("target size " + movements[movementFocus].targetSize);
                Debug.Log("length before " + movements[movementFocus].rotationSpeed.Length);
                if (movements[movementFocus].rotationSpeed.Length != movements[movementFocus].targetSize + 1)
                {
                    movements[movementFocus].rotationSpeed = new float[movements[movementFocus].targetSize + 1];
                    Debug.Log("length after " + movements[movementFocus].rotationSpeed.Length);
                }

                windowDisplay = new Rect(offsetX, offsetY, ELEMENT_DISPLAY, DISPLAY_HEIGHT);
                if (GUI.Button(windowDisplay, "Edit Targets"))
                {
                    Debug.Log("I am :" + movementFocus);
                    RecordData();
                    LookChainWindowEditor.Init(movementFocus, engine, ChainType.MOVEMENT);
                }
                offsetY = 120f;

                //Addup all the lock times.
                foreach(float time in movements[movementFocus].lockTimes)
                {
                    movements[movementFocus].movementTime += time;
                }
                //Addup all the rotation times
                foreach (float time in movements[movementFocus].rotationSpeed)
                {
                    movements[movementFocus].movementTime += time;
                }
                break;
    ...
    void FacingGUI()
    {
        ...
        switch (facings[facingFocus].facingType)
        {
            ...
            case FacingTypes.LOOKCHAIN:
                ...
                if (GUI.Button(windowDisplay, "Edit Targets"))
                {
                    Debug.Log("I am :" + facingFocus);
                    RecordData();
                    LookChainWindowEditor.Init(facingFocus, engine, ChainType.FACING);
                }
                offsetY = 120f;
                break;
    ...
    void TimeLineGUI()
    {
        ...
        GUIStyle tempStyle = new GUIStyle(EditorStyles.miniButtonMid);
        for (int i = 0; i < movements.Count; i++)
        {
            float tempTime = 0;
            switch (movements[i].moveType)
            {
                case MovementTypes.LOOKAT:
                case MovementTypes.LOOKCHAIN:
                    for (int k = 0; k < movements[i].rotationSpeed.Length; k++)
                    {
                        tempTime += movements[i].rotationSpeed[k];
                    }
                    for (int z = 0; z < movements[i].lockTimes.Length; z++)
                    {
                        tempTime += movements[i].lockTimes[z];
                    }
                    break;
                    //case MovementTypes.FREELOOK:
            ...
            switch(movements[i].moveType)
            {
                ...
                case MovementTypes.LOOKAT:
                case MovementTypes.LOOKCHAIN:
                    foreach (GameObject obj in movements[i].targets)
                    {
                        if (obj == null)
                        {
                            GUI.color = Color.red;
                    ...
                    foreach (float obj in movements[i].rotationSpeed)
                    {
                        if (obj <= 0)
                        {
                            GUI.color = Color.red;
                    ...
                    foreach (float obj in movements[i].lockTimes)
                    {
                        if (obj <= 0)
                        {
                            GUI.color = Color.red;
        ...
        for(int i = 0; i < facings.Count; i++)
        {
            ...
            {
                if (facingDraw - tempTime > displayLeft && tempTime > 0)
                {
                    //middle button
                    tempStyle = miniMid;
                    windowDisplay = new Rect(offsetX, offsetY, (600 / (displayRight - displayLeft) * tempTime), DISPLAY_HEIGHT);
